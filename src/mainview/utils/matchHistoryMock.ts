...
export function buildMockAiResult(predictedWinner: 'A' | 'B'): AiAnalysisResult {
  ...
  return {
    ...
    quickReasons: [
      l('A 队均分略高', 'Team A has a slightly higher average rating'),
      l('近期胜率更稳定', 'Recent win rate is more consistent'),
    ],
    keyFactors: [
      {
        side: 'A',
        type: 'strength',
        text: l('两队 ELO 接近，A 队小幅领先', 'The teams are close in ELO, with Team A slightly ahead'),
        weight: 0.55,
      },
      {
        side: 'Both',
        type: 'party',
        text: l('双方均有 2 人组排', 'Both teams have a two-stack'),
        weight: 0.25,
      },
    ],
    playerNotes: [
      {
        ...
        nickname: 'A1',
        side: 'A',
        text: l('模拟高评分突破手', 'Mock high-rated entry fragger'),
        role: 'entry',
      },
    ],
    risks: [l('本局为调试面板生成的模拟数据', 'This match uses mock data generated by the debug panel')],
    dataQuality: 'mock',
    teamSummary: {
      A: l('进攻节奏偏快', 'Faster attacking tempo'),
      B: l('防守站位较稳', 'More stable defensive positioning'),
    },
  };
}
