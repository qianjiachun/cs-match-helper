...
function releaseDate(value: string): string {
  return formatChangelogDate(value, currentLocale());
}
</script>
...
<template>
  <div class="space-y-3">
    <!-- 列表加载 -->
    <div v-if="listLoading" class="space-y-3" role="status" :aria-label="l('正在加载版本列表', 'Loading release list')">
      <div
        v-for="index in 4"
      ...
      <p class="flex items-center justify-center gap-2 py-2 text-[12px] text-fg-muted">
        ...
        {{ l('正在从 GitHub 获取版本列表…', 'Loading releases from GitHub…') }}
      </p>
    </div>
    ...
    >
      ...
      <p class="mt-3 text-[13px] font-medium text-fg">{{ l('无法加载更新日志', 'Could not load the changelog') }}</p>
    ...
    >
      <p class="text-[13px] text-fg-muted">{{ l('暂无公开发布记录', 'No public releases yet') }}</p>
    </div>
    ...
    >
      <header class="flex items-start gap-3 px-4 py-4">
        ...
        <div class="flex shrink-0 items-center gap-1">
          <button
            v-if="entry.htmlUrl"
            type="button"
            class="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-lg text-fg-muted transition-[color,background-color,transform] duration-200 hover:bg-elevated hover:text-fg active:scale-[0.96]"
            :aria-label="l('在 GitHub 查看发布', 'View release on GitHub')"
          ...
          <button
            type="button"
            class="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-lg text-fg-muted transition-[color,background-color,transform] duration-200 hover:bg-elevated hover:text-fg active:scale-[0.96]"
            :aria-label="expanded[entry.tagName] ? l('收起', 'Collapse') : l('展开', 'Expand')"
            @click="toggle(entry.tagName)"
          >
            <ChevronDown
      ...
      >
        <div
          ...
          class="space-y-2.5"
          role="status"
          :aria-label="l('正在加载更新详情', 'Loading release details')"
        >
          <div
            v-for="line in 3"
          ...
          <p class="flex items-center gap-2 pt-1 text-[12px] text-fg-muted">
            <Loader2 class="h-3.5 w-3.5 animate-spin text-accent" aria-hidden="true" />
            {{ l('正在加载更新详情…', 'Loading release details…') }}
          </p>
        </div>
        ...
        <template v-else-if="getDetailState(entry.tagName).status === 'ready'">
          ...
          <p v-else class="rounded-xl border border-dashed border-border bg-elevated/40 px-4 py-6 text-center text-[13px] text-fg-muted">
            {{ l('该版本暂无详细说明', 'No release notes are available for this version.') }}
          </p>
        </template>
      </div>
    </article>
