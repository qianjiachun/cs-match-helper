...
import { formatMatchInsight } from '@core/match/insights';
...
<template>
  <section v-if="insights" class="rounded-lg border border-border bg-elevated p-3">
    <div class="mb-2 flex items-center gap-2">
      ...
      <h3 class="text-[12px] font-semibold text-fg">{{ l('关键判断', 'Key takeaways') }}</h3>
      <span
        v-if="insights.strongerSide"
        class="ml-auto rounded bg-accent/10 px-2 py-0.5 text-[10px] font-medium text-accent"
      >
        {{ l(`优势方：队伍 ${insights.strongerSide}`, `Favored: Team ${insights.strongerSide}`) }}
      </span>
    </div>
    ...
    <ul v-if="insights.highlights.length > 0" class="mb-2 space-y-1">
      ...
      >
        · {{ formatMatchInsight(tip, currentLocale()) }}
      </li>
    </ul>
    ...
    <ul v-if="insights.risks.length > 0" class="space-y-1">
      ...
      >
        ...
        <span>{{ formatMatchInsight(tip, currentLocale()) }}</span>
      </li>
    </ul>
    ...
    >
      {{ l('双方数据接近，暂无显著差异提示', 'The teams are closely matched with no clear statistical edge.') }}
    </p>
  </section>
</template>
