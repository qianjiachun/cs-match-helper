...
const DEEPSEEK_BRAND_COLOR = '#5686FE';
...
const clipId = `deepseek-clip-${useId().replace(/:/g, '')}`;

const boxClass = computed(() =>
  props.size === 'sm' ? 'h-4 w-[calc(1rem*35/26)]' : 'h-[18px] w-[calc(18px*35/26)]',
);
</script>

<template>
  <span
    class="deepseek-icon inline-flex shrink-0 items-center justify-center leading-none"
    :class="[boxClass, muted ? 'opacity-45' : 'opacity-100']"
  >
    <svg
