...
const compareData = computed(() => {
  ...
  const aPlayers = teamA.value.players;
  const bPlayers = teamB.value.players;
  ...
  return {
    ...
    avgClutch: { label: '平均残局胜率', a: avgPlayerStat(aPlayers, 'clutchWinRate'), b: avgPlayerStat(bPlayers, 'clutchWinRate'), isPct: true, decimals: 0 },
  };
});
...
const teamARadar = computed(() => (teamA.value ? getTeamRadar(teamA.value.players) : null));
const teamBRadar = computed(() => (teamB.value ? getTeamRadar(teamB.value.players) : null));
...
<template>
  <div ref="boardRef" class="flex h-full min-h-0 flex-col gap-5 overflow-y-auto px-6 py-6">
      ...
      >
        ...
        <div class="flex flex-1 flex-col p-4">
          ...
          <div class="mb-2 text-center">
            <div class="mb-0.5 text-[12px] font-medium text-slate-500">队伍平均分</div>
      ...
      >
        ...
        <div class="mt-5 flex flex-col gap-2.5 border-t border-slate-100 pt-4">
          <div
            ...
            :class="{ 'border-b border-slate-100': key !== 'avgClutch' }"
          >
    ...
    <!-- 下排：指标卡片 -->
    <div v-if="compareData" class="grid shrink-0 grid-cols-6 gap-5">
      <div class="compare-entrance-stat stat-card flex flex-col justify-between rounded-2xl border border-slate-200/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
        <div>
          <div class="mb-4 flex items-center gap-2 text-[13px] font-bold text-slate-600">
            <div class="flex h-6 w-6 items-center justify-center rounded-md bg-indigo-50 text-indigo-500">
              <Target class="h-3.5 w-3.5" />
            </div>
            ADR
          </div>
          <div class="mb-3 flex items-center justify-between font-black">
            <span class="text-xl text-blue-600">{{ formatNum(compareData.avgAdpr.a, 1) }}</span>
            <span class="text-[11px] font-bold text-slate-300">VS</span>
            <span class="text-xl text-orange-500">{{ formatNum(compareData.avgAdpr.b, 1) }}</span>
          </div>
        </div>
        <div>
      ...
      <div class="compare-entrance-stat stat-card flex flex-col justify-between rounded-2xl border border-slate-200/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
        <div>
          ...
          <div class="mb-3 flex items-center justify-between font-black">
            ...
            <span class="text-[11px] font-bold text-slate-300">VS</span>
            <span class="text-xl text-orange-500">{{ formatNum(compareData.avgRating.b, 2) }}</span>
          </div>
        </div>
        <div>
          <div class="relative mb-2.5 flex h-1.5 overflow-hidden rounded-full bg-slate-100 shadow-inner">
      ...
      <div class="compare-entrance-stat stat-card flex flex-col justify-between rounded-2xl border border-slate-200/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
        <div>
          <div class="mb-4 flex items-center gap-2 text-[13px] font-bold text-slate-600">
            <div class="flex h-6 w-6 items-center justify-center rounded-md bg-emerald-50 text-emerald-500">
              <Crosshair class="h-3.5 w-3.5" />
            </div>
            K/D
          </div>
          <div class="mb-3 flex items-center justify-between font-black">
            <span class="text-xl text-blue-600">{{ formatNum(compareData.avgKd.a, 2) }}</span>
            <span class="text-[11px] font-bold text-slate-300">VS</span>
            <span class="text-xl text-orange-500">{{ formatNum(compareData.avgKd.b, 2) }}</span>
          </div>
        </div>
        <div>
          <div class="relative mb-2.5 flex h-1.5 overflow-hidden rounded-full bg-slate-100 shadow-inner">
      ...
      <div class="compare-entrance-stat stat-card flex flex-col justify-between rounded-2xl border border-slate-200/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
        <div>
          <div class="mb-4 flex items-center gap-2 text-[13px] font-bold text-slate-600">
            <div class="flex h-6 w-6 items-center justify-center rounded-md bg-rose-50 text-rose-500">
              <Skull class="h-3.5 w-3.5" />
            </div>
            爆头率
          </div>
          <div class="mb-3 flex items-center justify-between font-black">
            <span class="text-xl text-blue-600">{{ formatPct(compareData.avgHsRate.a) }}</span>
            <span class="text-[11px] font-bold text-slate-300">VS</span>
            <span class="text-xl text-orange-500">{{ formatPct(compareData.avgHsRate.b) }}</span>
          </div>
        </div>
        <div>
          <div class="relative mb-2.5 flex h-1.5 overflow-hidden rounded-full bg-slate-100 shadow-inner">
      ...
      <div class="compare-entrance-stat stat-card flex flex-col justify-between rounded-2xl border border-slate-200/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
        <div>
          <div class="mb-4 flex items-center gap-2 text-[13px] font-bold text-slate-600">
            <div class="flex h-6 w-6 items-center justify-center rounded-md bg-cyan-50 text-cyan-500">
              <UserCheck class="h-3.5 w-3.5" />
            </div>
            首杀成功率
          </div>
          <div class="mb-3 flex items-center justify-between font-black">
            <span class="text-xl text-blue-600">{{ formatPct(compareData.avgFirstKill.a) }}</span>
            <span class="text-[11px] font-bold text-slate-300">VS</span>
            <span class="text-xl text-orange-500">{{ formatPct(compareData.avgFirstKill.b) }}</span>
          </div>
        </div>
        <div>
          <div class="relative mb-2.5 flex h-1.5 overflow-hidden rounded-full bg-slate-100 shadow-inner">
      ...
      <div class="compare-entrance-stat stat-card flex flex-col justify-between rounded-2xl border border-slate-200/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
        <div>
          <div class="mb-4 flex items-center gap-2 text-[13px] font-bold text-slate-600">
            <div class="flex h-6 w-6 items-center justify-center rounded-md bg-purple-50 text-purple-500">
              <Flag class="h-3.5 w-3.5" />
            </div>
            残局胜率
          </div>
          <div class="mb-3 flex items-center justify-between font-black">
            <span class="text-xl text-blue-600">{{ formatPct(compareData.avgClutch.a) }}</span>
            <span class="text-[11px] font-bold text-slate-300">VS</span>
            <span class="text-xl text-orange-500">{{ formatPct(compareData.avgClutch.b) }}</span>
          </div>
        </div>
        <div>
          <div class="relative mb-2.5 flex h-1.5 overflow-hidden rounded-full bg-slate-100 shadow-inner">
