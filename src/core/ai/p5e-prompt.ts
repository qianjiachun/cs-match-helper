...
- kd = 赛季 K/D（season_data.kill / death），无 home 时回退 match detail fight
- seasonWinRate = 本赛季优先排位胜率（season_data.per_win_match）
- recentRatings = 近若干场 ELO 变化序列（elo specialData），不是单场 Rating 列表
- mapWinRate / mapTotalNum / mapSampleLow = 当前地图样本与胜率（map-ext），非赛季本图统计；仅当 mapStatus=ready 时存在
- matchDetailStats.fight = 当局单场表现（赛后才有），不要与 seasonRating 混淆
...
function recentEloChangesForUuid(bundle: P5eMatchBundle, uuid: string): number[] | undefined {
  const body = bundle.eloInfo?.responseBody;
  if (!body || typeof body !== 'object') return undefined;
  const data = (body as Record<string, unknown>).data;
  if (!data || typeof data !== 'object') return undefined;
  const row = (data as Record<string, unknown>)[uuid];
  ...
  const modeInfo = (modes as Record<string, unknown>)[modeKey];
  if (!modeInfo || typeof modeInfo !== 'object') return undefined;
  const special = parseP5eSpecialData((modeInfo as Record<string, unknown>).specialData);
  return special.recentRatings.length ? special.recentRatings.slice(-5) : undefined;
}
...
function summarizeP5ePlayer(
  ...
  return compact({
    ...
    seasonRating: round(player.seasonRating),
    seasonAdr: round(player.adpr, 1),
    seasonRws: round(player.weRaw, 1),
    seasonWinRate: formatPct(player.seasonWinRate),
    recentRatingAvg: round(player.rating),
    mapRating: round(player.seasonRating),
    kd: round(player.kd),
    ...
    recentMatchRatings: player.recentRatings.length ? player.recentRatings.slice(0, 5) : undefined,
    recentEloChanges: bundle && uuid ? recentEloChangesForUuid(bundle, uuid) : undefined,
...
function assessP5eDataQuality(record: MatchRecord, bundle?: P5eMatchBundle) {
  const mapStatus = resolveP5eMapStatus(record);
  const missing: string[] = [];
  if (!bundle?.userInfo) missing.push('userInfo');
  if (!bundle?.eloInfo) missing.push('eloInfo');
  if (!bundle?.mapExt) missing.push('mapExt');
  const homeCount = bundle?.playerHome ? Object.keys(bundle.playerHome).length : 0;
  const homePartial =
    bundle && homeCount > 0 && homeCount < bundle.uuids.length ? true : undefined;
  const homeMissing = bundle && homeCount === 0 ? true : undefined;
  ...
  return compact({
    ...
    homeDataPartial: homePartial,
    homeDataMissing: homeMissing,
    hasMatchDetail,
    officialTeams,
    ...
    mapStatus,
    mapUnknown: mapStatus === 'unknown' ? true : undefined,
    mapInferred: mapStatus === 'ready' && !hasMatchDetail ? true : undefined,
...
function summarizeP5eMatchContext(record: MatchRecord, bundle?: P5eMatchBundle) {
  ...
  return compact({
    ...
    mapStatus: resolveP5eMapStatus(record),
    mapName: resolveP5eMapName(record),
