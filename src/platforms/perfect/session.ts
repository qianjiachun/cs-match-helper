...
export function mergePerfectStats(player: MatchPlayer, stats: PerfectPlayerStats): MatchPlayer {
  const apiScore = stats.pvpScore;
  const currentScore = player.score;
  const score = currentScore != null && currentScore > 0
    ? currentScore
    : apiScore ?? currentScore;
  const hasAbilityProfile = Boolean(
    stats.abilityProfile
    && Object.values(stats.abilityProfile).some((value) => value != null && value !== ''),
  );
  const hasCombat = Boolean(stats.combat && Object.keys(stats.combat).length);
  return {
    ...player,
    nickname: stats.name ?? player.nickname,
    avatar: stats.avatar ?? player.avatar,
    platformBoardId: stats.zqId ?? player.platformBoardId,
    score,
    currentSStars: stats.currentSStars ?? player.currentSStars,
    peakScore: stats.allSeasonMaxScore ?? player.peakScore,
    peakSStars: stats.allSeasonMaxStars ?? player.peakSStars,
    peakSeason: stats.allSeasonMaxScoreSeason ?? player.peakSeason,
    seasonTotalNum: stats.seasonMatches ?? player.seasonTotalNum,
    kd: stats.kd ?? player.kd,
    seasonWinRate: stats.winRate ?? player.seasonWinRate,
    seasonWinNum: stats.seasonWinNum ?? player.seasonWinNum,
    standardRating: stats.standardRating ?? player.standardRating,
    recentStandardRating: stats.recentStandardRating ?? player.recentStandardRating,
    commonRating: stats.commonRating ?? player.commonRating,
    seasonRating: stats.pwRating ?? player.seasonRating,
    rating: stats.recentPwRating ?? player.rating,
    recentRatings: stats.recentPwRatings.length ? stats.recentPwRatings : player.recentRatings,
    rws: stats.rws ?? player.rws,
    recentRws: stats.recentRws ?? player.recentRws,
    adpr: stats.adr ?? player.adpr,
    hsRate: stats.headShotRatio ?? player.hsRate,
    firstKillSuccessRate: stats.firstKillSuccessRate ?? player.firstKillSuccessRate,
    rapidStopSuccessRate: stats.rapidStopSuccessRate ?? player.rapidStopSuccessRate,
    reactionTime: stats.reactionTime ?? player.reactionTime,
    entryKillRatio: stats.entryKillRatio ?? player.entryKillRatio,
    clutchWinRate: stats.vs1WinRate ?? player.clutchWinRate,
    clutch1v1Rate: stats.clutch1v1Rate ?? player.clutch1v1Rate,
    clutch1v1Attempts: stats.clutch1v1Attempts ?? player.clutch1v1Attempts,
    kast: stats.kast ?? player.kast,
    tradeFragRate: stats.tradeFragRate ?? player.tradeFragRate,
    roundMvpCount: stats.roundMvpCount ?? player.roundMvpCount,
    combat: hasCombat ? stats.combat : player.combat,
    weAvg: stats.recentWe ?? player.weAvg,
    seasonWe: stats.avgWe ?? player.seasonWe,
    eloTrend: stats.eloTrend ?? player.eloTrend,
    kills: stats.kills ?? player.kills,
    deaths: stats.deaths ?? player.deaths,
    assists: stats.assists ?? player.assists,
    mvpCount: stats.matchMvpCount ?? stats.mvpCount ?? player.mvpCount,
    clutchWins: stats.clutchWins ?? player.clutchWins,
    clutch1v1: stats.clutch1v1 ?? player.clutch1v1,
    clutch1v2: stats.clutch1v2 ?? player.clutch1v2,
    clutch1v3: stats.clutch1v3 ?? player.clutch1v3,
    clutch1v4: stats.clutch1v4 ?? player.clutch1v4,
    clutch1v5: stats.clutch1v5 ?? player.clutch1v5,
    clutch1v1Total: stats.clutch1v1Total ?? player.clutch1v1Total,
    clutch1v2Total: stats.clutch1v2Total ?? player.clutch1v2Total,
    clutch1v3Total: stats.clutch1v3Total ?? player.clutch1v3Total,
    clutch1v4Total: stats.clutch1v4Total ?? player.clutch1v4Total,
    clutch1v5Total: stats.clutch1v5Total ?? player.clutch1v5Total,
    multiKill2: stats.multiKill2 ?? player.multiKill2,
    multiKill3: stats.multiKill3 ?? player.multiKill3,
    multiKill4: stats.multiKill4 ?? player.multiKill4,
    multiKill5: stats.multiKill5 ?? player.multiKill5,
    hotMaps: stats.hotMaps.length ? stats.hotMaps : player.hotMaps,
    abilityProfile: hasAbilityProfile ? stats.abilityProfile : player.abilityProfile,
    radar: stats.radar ? { ...player.radar, ...stats.radar } : player.radar,
    primaryWeapons: stats.primaryWeapons.length ? stats.primaryWeapons : player.primaryWeapons,
    perfectLoadState: {
      ...player.perfectLoadState!,
      stats: 'loaded',
      statsError: stats.partialFailure,
    },
  };
}
  ...
  apply(event: PerfectMatchEvent, logLine: LogLine, now = Date.now()): PerfectSessionUpdate {
    ...
    if (event.kind === 'game-start') {
      ...
      for (const raw of playersFromGameInfo(event.gameInfo)) {
        ...
        const rawScore = numberValue(raw.score);
        if (!known.has(steamId)) result.newPlayerIds.push(steamId);
        known.set(steamId, {
          ...previous,
          score: rawScore != null && rawScore > 0 ? rawScore : previous.score ?? rawScore,
